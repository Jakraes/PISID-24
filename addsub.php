...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if(isset($_POST['nome']) && isset($_POST['sala'])){
        ...
        if ($result && mysqli_num_rows($result) > 0) {
            ...
            // Insert the substancia with the fetched experience ID
            $sql = "INSERT INTO substancia (Nome, Numero_Ratos, ID_Experiencia) VALUES ('$nome', '$sala', '$id_experiencia')";
            if (mysqli_query($conn, $sql)) {
                echo "<script>alert('Substância gravada com sucesso!');</script>";
            } else {
                echo "Erro ao gravar a substância: " . mysqli_error($conn);
            }
        } else {
...
<head>
    <meta charset="UTF-8">
    <title>Adicionar substancia - TECH MICE</title>
...
<body>
    <div class="general-container">
        ...
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" onsubmit="return confirmarGravacao()" class="form-container">
            ...
            <input type="number" id="sala" name="sala" placeholder="Número de ratos" required>

            <div class="action-buttons">
                <button type="button" onclick="voltar()">Voltar</button>
                <button type="submit" name="action" value="Gravar substancia">Gravar substância</button>
            </div>
        </form>
    </div>
    <script>
        ...
        function confirmarGravacao() {
            var confirmado = confirm('Você confirma a gravação desta substância?');
            if (confirmado) {
